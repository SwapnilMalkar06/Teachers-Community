...
import { FileText, Download, Eye, Search, Filter, X, Sparkles } from 'lucide-react';
...
export default function NotesPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const res = await fetch('/api/teaching/resources?type=NOTES');
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-16">
      ...
      <div className="bg-gradient-to-b from-sky-50/70 via-white to-slate-50 py-12 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-slate-600 mt-2 max-w-3xl font-medium">
            Unit-wise lecture notes prepared by Prof. Ashwini Sawant. View PDFs inline or download for offline studying.
          </p>
        </div>
      </div>
      ...
      {/* Notes PDF Cards Grid */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {loading ? (
          <div className="text-center py-12 text-slate-500 font-medium">Loading lecture notes...</div>
        ) : filteredResources.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 space-y-2">
            <FileText className="w-10 h-10 text-slate-400 mx-auto" />
            <h3 className="text-lg font-bold text-slate-800">No Lecture Notes Found</h3>
            <p className="text-xs text-slate-500">Try selecting a different subject or clearing your search filter.</p>
          </div>
        ) : (
              ...
              >
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-slate-600 line-clamp-3 leading-relaxed">
                    {note.description || 'Unit lecture notes covering core concepts, solved examples, and diagrams.'}
                  </p>
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center space-x-2">
                  <button
                    onClick={() => setPreviewPdfUrl(note.fileUrl)}
          ...
          <div className="bg-white w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center space-x-2 font-bold text-sm">
                ...
                <span>Lecture Notes Viewer</span>
              </div>
              <button
